...
int BigInt::GetInt()
{
	IntList temp = intAsList;
	int count = temp.size();
	int number, converted = 0;
	int i = 0;
	for (std::list<uchar>::reverse_iterator rit = intAsList.rbegin(); rit != intAsList.rend(); ++rit)
	{
		number = static_cast<int>(*rit);
		number *= pow(10, i);
		converted += number;
		i++;
	}
...
BigInt BigInt::bigExp(BigInt base, BigInt exp)
{
	if (exp < 0) {
		throw new invalid_argument("Error, exponent is negative please change");
	}
	if (exp == 0) {
		return 1;
	}
	...
	{
		return (base * (bigExp((base*base), (exp - 1) / 2)));
	}
	else
	{
		//exponent even
...
{
	this->intAsList.push_back(digit);
	return *this;
}

BigInt BigInt::clear()
{
	this->intAsList.clear();
	return *this;
}
...
BigInt BigInt::half() {
	...
	while (result.intAsList.front() == 0) { //result.intAsList.front() == 0 && intAsList.size != 0
		result.intAsList.pop_front();
		if (result.intAsList.size() == 0)
		{
			break;
		}
	}
...
{
	BigInt num1(500);
	int temp = num1.GetInt();
	//cout << num << num.GetInt() << endl; //integer << not working for some reason so much easier to use breakpoints to test
	BigInt num2(489);				 //Could be broken due to MSVC++ compiler or BigInt class overload<< might be confused
	temp = num2.GetInt();			//I am using BigInt.h from student that was downloaded from Algorithms website
	BigInt num3(1832);
	temp = num3.GetInt();
	BigInt num4(701);
	temp = num4.GetInt();
...
void bigExpTest()
{
	...
	BigInt num2(10);
	BigInt num3(76);
	cout << num.bigExp(num, 2) << " should be 4" << endl;
	cout << num2.bigExp(num2, 5) << " should be 100,000" << endl;
	cout << num3.bigExp(num3, 12) << " should be 37133262473195501387776" << endl;
	//see if it can handle a big one
	BigInt num4(14858);
	//cout << num4.bigExp(num4, 109) << " should be very large" << endl; worked but takes a few seconds
	int pause;
	cin >> pause;
}
